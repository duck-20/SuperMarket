...
namespace Plugins.DataStore.SQL
{
	public class TransactionSQLRepository : ITransactionRepository
	{
		...
		public IEnumerable<Transaction> GetByDayAndCashier(string cashierName, DateTime date)
		{
			...
			{
				return db.Transactions.Where(x=>EF.Functions.Like(x.CashierName, cashierName)&&x.TimeStamp.Date==date.Date);
			}
		}
		...
		public IEnumerable<Transaction> Search(string cashierName, DateTime startDate, DateTime dateTime)
		{
			if (string.IsNullOrWhiteSpace(cashierName))
			{
				return db.Transactions.Where(x => x.TimeStamp.Date >= startDate.Date && x.TimeStamp.Date<=dateTime.Date);
			}
			else
			{
				return db.Transactions.Where(x => EF.Functions.Like(x.CashierName, cashierName) && x.TimeStamp.Date >= startDate.Date && x.TimeStamp.Date <= dateTime.Date);
			}
		}
	}
}
